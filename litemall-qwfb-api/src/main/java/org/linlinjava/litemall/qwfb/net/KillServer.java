...
public class KillServer {
    ...
    public void start(int port) {
        ...
        String operationSystem = props.getProperty("os.name");

        String cmd = "cmd /c netstat -ano | findstr \"\" + port + \"\"";
        if (operationSystem.indexOf("Mac") >= 0) {
            killMac(runtime, port);
        } else if (operationSystem.indexOf("Linux") >= 0) {
            cmd = "netstat -lntp | grep " + port;
        } else {
            killWindow(runtime, port);
        }
    }

    public void killMac(Runtime runtime, int port) {
        try {
            ...
            if (read.size() == 0) {
                ...
                for (String portLine : read) {
                    if (portLine.indexOf("COMMAND") >= 0 || portLine.indexOf("java") < 0
                            || portLine.indexOf("LISTEN") < 0) {
                        continue;
                    }
                    portLine = portLine.replace("java", "").trim();
                    portLine = portLine.substring(0, portLine.indexOf(" "));
                    ports.add(portLine);
                }
                System.out.println("找到" + ports.size() + "个进程，正在准备清理" + ports);
    ...
    public void killWindow(Runtime runtime, int port) {
        try {
            String cmd = "cmd /c netstat -ano | findstr \"\" + port + \"\"";
